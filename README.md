...
[ Vue.js 前端 (MPA) ]
        │
        └──────────────► [ Python Flask ]          偵測 / 統計 / 網址 / 電話 / 警示牆 / AI問答 / 模擬遊戲
                                   │
                          [ PostgreSQL ]            訓練資料 / 問答字典 / 歷史詐騙資料 2021–2025
```

全部部署於 **render**（前端 Static Site + 單一 Flask Web Service + render PostgreSQL）。
...
| `backend-api/` | 偵測（自訓模型 + Gemini）/ 統計 / 網址 / 電話 / 警示牆 / AI問答 / 模擬遊戲 API | Python + Flask + scikit-learn |
...
未設定 `DATABASE_URL` 時，後端自動 fallback 用 `db/fixtures.json`；未設定 `GEMINI_API_KEY` 時，偵測走規則 fallback。兩個服務各開一個終端機：

```bash
# 1) Flask 後端（偵測 / 統計 / 網址 / 電話 / 警示牆 / AI問答 / 模擬遊戲）
cd backend-api && pip install -r requirements.txt
gunicorn app.flask_app:app              # → http://localhost:8000
...
接真實資料時：用 `docker compose up -d db` 起本機 Postgres（或填 render 連線字串到後端 `.env`），再 `db/schema.sql` + `db/seed.sql`，並設 `GEMINI_API_KEY` 啟用 Gemini 偵測。

## 開發

各子專案的啟動方式見各自的 `README.md`。

## 文件

- 專題計畫書：`docs/`（原始檔 `專題計畫書.docx`）
- 架構決策與開發歷程：見《專題計畫書》「架構決策摘要」「開發歷程」章
- 成員分工：`TEAM.md`
